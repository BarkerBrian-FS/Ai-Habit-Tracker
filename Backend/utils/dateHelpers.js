...
export const calcStreak = (sordedDateKeys) => {
    if(!sordedDateKeys.length) return { current: 0, longest: 0 };
    const set = new Set(sordedDateKeys);
    ...
    const sortedAsc = [...sordedDateKeys].sort();
    let longest = 0;
    let run = 0;
    let prev = null;
